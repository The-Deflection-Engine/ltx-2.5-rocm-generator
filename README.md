...
**v0.2.0** — shown in the GUI title bar and via `python cli_gen_vid.py --version`.
...
| `tests/ab_render.py` | Runs a set of config variants at a fixed seed and collects timings/VRAM. |
| `tests/test_vram_calibration.py` | Self-check for the VRAM fit and first-run defaults. No GPU needed. |

Both front-ends share the same `ltx2_config.json` and the same pipeline, so a fix/change in one reaches both.

See [`Examples/`](Examples/) for real prompts paired with the clips they produced, and [`Prompting Guide/`](<Prompting Guide/>) for how to write one.
...
* **Full GPU-resident mode on large cards:** by default the transformer streams block-by-block from pinned host RAM (group offload), which is what a 16GB card needs. On a card with **≥29GB VRAM** *and* a run small enough to fit alongside the resident weights, it's kept fully GPU-resident instead, skipping the PCIe streaming cost entirely. Both halves are automatic — no setting to flip. **Measured on a 32GB card:** the ~18GB transformer stays resident through the VAE decode, which is the peak of the whole run, so 32GB leaves only ~7,700 tokens for the render itself — about 105 frames at 1024×576. Longer than that and it falls back to streaming and says so in the log, rather than OOMing at the decode after paying for every denoise step. `FULL_RESIDENT_VRAM_THRESHOLD_GB` is the floor at which resident mode is ever worth *trying*; running full-length clips resident wants ~40GB, not 29.
...
* **Prompt enhancer (optional), with a model picker:** click **✨ Enhance Now** to rewrite a short prompt into the long, detailed caption style LTX-2.5 was trained on. Choose between `google/gemma-4-E4B-it` (default, more accurate) and `E2B` (faster, ~9.6GB vs ~15GB) in the **Enhancer model** dropdown; if the one you pick isn't downloaded, it offers to fetch it and streams the download into the log. The result replaces the prompt box, so you read and edit it before spending minutes on a render — no hidden rewrites. It runs in a throwaway subprocess and frees itself on exit, so it costs **zero resident VRAM or RAM during generation**. Conditioned on your reference frame for image-to-video, and on **both** frames for First+Last Frame mode.
...
* **First+Last Frame → Video (FLF2V):** give a start image *and* an end image and the model generates the clip between them. LTX's own name for this is FLF2V, and it needs the keyframe-aware `LTX2ConditionPipeline` rather than the frame-0-only image-to-video one — so it is its own Generation Mode, not a tickbox on Image → Video. The end frame is pinned by *latent* index `-1`, so it lands on the true last frame whatever length gets generated. The prompt still matters: the two images fix appearance at each end, but only the prompt says what happens in between. **✨ Enhance Now** understands this mode and is shown both frames.
* **IC-LoRA video-to-video — the real one:** your reference clip is *appended to the token sequence* as extra tokens the transformer attends to, so structure and motion follow the reference while the prompt drives appearance. This is what Lightricks means by video-to-video (their docs: *"Video Conditioning (ICLoraPipeline only)"*), and it needs an IC-LoRA trained to attend across reference↔target — pick one in the LoRA box. Verified here with `LTX-2.3-22b-IC-LoRA-Union-Control` driven by a canny edge pass: pose, mic and guitar placement tracked the control signal exactly while palette and lighting came from the prompt. Note most **control** IC-LoRAs want a control signal (canny/depth/pose) as the reference, not your raw clip — check the adapter's model card. Single-stage only and no Auto Duration (the reference rides the same sequence; length should match it). The `reference_downscale_factor` is read from the adapter's own metadata, which matters — diffusers defaults it to 1 and never reads it, and a wrong value misplaces every reference token.
* **Video-to-video (the crossfade one):** feeds a clip into `LTX2ConditionPipeline`/`LTX2VideoCondition` at `index=0`, with a **strength** value that runs the *opposite* direction from img2img denoise strength: `1.0` keeps the source untouched, `0.0` is fully noised. Its useful range is very narrow, and now understood rather than merely observed: the condition spans the whole clip so every token gets the same mask, and the denoise loop re-blends `x0 = model*(1-s) + source*s` on **every** step, so the model's free contribution decays as `(1-strength)^steps`. Measured at a fixed seed, outputs at `0.4` and `1.0` are 98.4% identical, `0.6` and `1.0` are 99.4% — a near-step function, not a ramp. Anything from ~`0.2` up is effectively just the source. It is a crossfade toward the source, not a restyle dial, and no tuning makes it one — **for actual restyling use IC-LoRA video-to-video above.** Useful settings are the extremes: very low (real change, little source survives) or ~`0.9`-`1.0` (source dominates, prompt nudges a small edit). Requires `imageio` (see [Requirements](#requirements)).
* **LoRA loading (on by default):** points at a local `.safetensors`/`.pt`/`.bin` LoRA file and applies it on top of the base model at pipeline build time. Confirmed working for a plain LoRA (Cinemagraph — real, visible effect: locked composition vs. the base model's normal camera drift) **and** for IC-LoRAs, which now have a mode of their own (above). Measured against `LTX-2.3-22b-IC-LoRA-Union-Control`: diffusers already converts Lightricks' key format, all 960 tensors, no shim — and a 2.3-22b adapter is structurally compatible with the 2.5 transformer (every targeted module present, blocks 0-47 aligned, all A/B pairs shape-clean), which is consistent with LTX's own 2.5 ComfyUI workflows loading the 2.3 adapters. There is no universal trigger-phrase prompt format; it is per-adapter (Day-To-Night takes a plain descriptive prompt, Ingredients a two-part one) — read the model card. Requires `peft` (see [Requirements](#requirements)).

  **Stack several at once.** The LoRA panel is an ordered list — Add (multi-select in the dialog), Remove, Clear, and a scale spinbox that edits the selected row. Each adapter loads under its own name and they compose, with per-adapter weights; one that fails to load is skipped and the rest still apply. On the CLI, repeat `--lora` (and optionally `--lora-scale`, matched positionally). Adding, removing, reordering or re-scaling anything rebuilds the pipeline, since the adapters are merged into the resident transformer — a scale change alone counts.

  **The stack starts empty every launch**, deliberately: a LoRA is a per-experiment choice, and silently inheriting the last session's adapters is the sort of thing you notice three renders later. The `loras` list is still written to `ltx2_config.json` on Generate so a run stays reproducible from it — it is simply never read back at startup.

  **A LoRA applies in every mode**, since the weights merge into the transformer that all modes share. The exception is an **IC-LoRA**: its weights load anywhere, but what they encode is *"when reference tokens are present, attend to them"* — and outside IC-LoRA Video → Video there are none, so it silently does nothing. The GUI spots that case and warns inline. Detection is a heuristic (the adapter's `reference_downscale_factor` metadata, else an `ic-lora` filename), because an IC-LoRA is structurally an ordinary LoRA — so it only ever warns, never blocks.
* **Modality guidance (off by default):** steers away from a prediction made with audio↔video cross-attention switched off, which tightens how much the soundtrack is derived from what is on screen. One extra pass (~2x), same cost shape as STG. Measured at a fixed seed: the **video** barely changes (correlation 0.93) while the **audio** is almost entirely different (0.20) — it is an audio/AV-coupling knob, not a picture one. Set `modality_scale` to `3.0` in `ltx2_config.json` to try it. Note this is off-recipe: `3.0` is upstream's value but belongs to their *guided* presets, and their `DistilledPipeline` — the recipe this app runs — applies no guidance at all, so it never runs modality guidance on the 8-step schedule. Which of the two soundtracks is better is a listening call that hasn't been made under controlled conditions.
* **Self-calibrating VRAM model:** the token warning, the pre-flight estimate and Auto Duration's cap all key off `VRAM_GB ≈ base + per_token × tokens`, which shipped as a two-point fit from one RX 9070 XT. Every successful run now appends its own effective-token count and process-local peak VRAM to `vram_calibration.json` (per-machine, untracked) and the model is refitted from what *your* card actually did. Measured here across 5 runs spanning 448-12,480 tokens including a 2-stage one, the refit sits within **±0.14GB** of every observation, where the shipped constants were 13% high at 6,630 tokens. It refuses to fit rather than guess — needs 3+ points, a 1.5x token span, a largest run of ≥5,000 tokens, and a sane result — so a handful of 256×256 drafts can't set the ceiling for full-size renders. Keyed by offload profile, because a card past the full-resident threshold has a completely different intercept.
* **First-run defaults sized to your GPU:** on a fresh install (no `ltx2_config.json`) the opening resolution is derived from the calibrated model rather than assuming 16GB — roughly 320×192 at 8GB, 512×288 at 10GB, and 960×544 from 12GB up. It announces what it picked.

  It is capped at 960×544, the documented LTX-2.5 reference base, rather than the largest thing your card could hold — so a 48GB card opens on the same size as a 16GB one. A 16GB card *can* start at 1280×704 (measured: 14,080 tokens, 9.21GB peak) but that render takes about 7 minutes against roughly half for 960×544, and a default should be the quick, safe starting point rather than the most the hardware will take. Raise it in one click; the headroom is yours to spend deliberately.

  **Resolution is all it sets:** upscale, STG, CFG and modality guidance stay off however much VRAM is spare, because each one either changes the output format or multiplies cost, and none of them should switch themselves on because your card is roomy. Once you've clicked Generate the saved config wins permanently — this never reaches in and changes a setting you chose.
...
`LTX2ConditionPipeline` the crossfade video-to-video already uses under the
hood, just with several indexed conditions instead of one — technically close,
but the UI for authoring multiple shots is its own piece of work, not yet
built.

Known divergence from the reference recipe: upstream's `DistilledPipeline`
switches **stage 1 to an ancestral (SDE) Euler sampler** for checkpoints at
2.5 or newer (`ANCESTRAL_SAMPLER_SINCE_VERSION` in their `distilled.py`),
keeping stage 2 deterministic. This app runs deterministic Euler throughout,
because that is what `diffusers` does and what this checkpoint's own
`scheduler_config.json` asks for (`"stochastic_sampling": false`). Flipping
that flag does *not* close the gap: diffusers' stochastic branch uses a
different update rule (it discards the current sample and redraws from the
forward process, where upstream steps to `sigma_down = s_next²/s` and renoises
variance-preservingly), and no LTX-2 pipeline in `diffusers` passes a
`generator` to `scheduler.step()`, so enabling it would break same-seed
reproducibility. Recorded as a recipe difference; whether it would improve
output here is untested.
...
python cli_gen_vid.py --image a.png --end-image b.png          # first+last frame (FLF2V)
python cli_gen_vid.py --video clip.mp4 --video-strength 0.05  # video-to-video (crossfade) -- see Key Features
python cli_gen_vid.py --video canny.mp4 --ic-v2v --lora union-control.safetensors  # IC-LoRA video-to-video
python cli_gen_vid.py --lora style.safetensors --lora-scale 0.8  # on by default -- see Key Features
python cli_gen_vid.py --lora a.safetensors --lora-scale 0.8 --lora b.safetensors  # stack several
```
...
* **AMD GPU with ROCm support, bfloat16, and enough VRAM.** Developed and tested on exactly one card — an RX 9070 XT (gfx1201). Nothing here is architecture-specific, so other ROCm-capable AMD GPUs *should* work, but that is untested and not a promise. Two things decide it:
  ...
  * **VRAM is the binding constraint.** The fixed footprint is ~8GB before any activations, and the rest scales with `latent_frames × (H/32) × (W/32)`:
  ...
  Those figures come from a model originally fitted on one GPU (see [Tuning knobs](#tuning-knobs-in-ltx2_configjson)), so treat them as a starting guide rather than a specification — **but the app no longer stays stuck with them.** Every successful run feeds its own token count and peak VRAM back in, and after a few real-size runs the threshold is refitted to your card (see *Self-calibrating VRAM model* under Key Features). On the development card the refit moved the ceiling from ~29,000 to ~33,800 tokens, i.e. the shipped numbers were slightly *over*-cautious there.
...
* *(Optional, for the prompt enhancer)* a Gemma-4 checkpoint. The LTX-2.5 checkpoints ship `prompt_enhancer` and `processor` as nulls, so this is a separate download — but **you no longer need to fetch it by hand**: pick a model in the GUI's **Enhancer model** dropdown and click **✨ Enhance Now**, and it offers to download the one you chose. Both are ungated and need no HF token. To pre-fetch instead:
  ```bash
  hf download google/gemma-4-E4B-it --local-dir local_ltx25_enhancer_e4b   # default, ~15GB
  hf download google/gemma-4-E2B-it --local-dir local_ltx25_enhancer       # faster, ~10GB
  ```
* *(Optional, for IC-LoRA video-to-video)* an IC-LoRA adapter, e.g. [`Lightricks/LTX-2.3-22b-IC-LoRA-Union-Control`](https://huggingface.co/Lightricks/LTX-2.3-22b-IC-LoRA-Union-Control) (~0.65GB, ungated — canny/depth/pose in one). Several of Lightricks' effect adapters (Relight, Day-To-Night) are gated and need an access request on HF. Point the LoRA box at the `.safetensors` file; nothing else to configure, as the reference downscale factor is read from the file itself.
* **All optional dependencies ship in `requirements.txt` anyway** (`imageio` for video input, `peft` for LoRAs) — a plain `pip install -r requirements.txt` covers every mode, nothing extra to fetch when you turn one on.
