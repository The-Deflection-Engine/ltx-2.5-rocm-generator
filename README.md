...
* **Working two-stage upscale:** generate at a lower base resolution, then run a 2x latent upsample + short refinement pass at the target resolution — faster than generating at full resolution directly, with one noise generator threaded through both stages per the LTX-2.5 reference recipe.
...
The binding constraint is VRAM, not RAM — the transformer's attention cost scales with `latent_frames × (H/32) × (W/32)`, which the script tracks as a "token count" and warns you about above ~50,000 (with the option to continue anyway).

| Mode | Base resolution | Frames | Final output | Notes |
|---|---|---|---|---|
| **Default / recommended** | 960x544 (2-stage) | 121–161 (5–6.7s @24fps) | 1920x1088 | Comfortably within 16GB VRAM |
| Single-stage, no upscale | 1280x704 | up to ~121f | 1280x704 | Skip the checkbox for a direct full-res render |
| Pushing it | 960x544 (2-stage) | up to ~193f (8s) | 1920x1088 | Near the VRAM ceiling; expect the token warning |

Going meaningfully past these (e.g. full-res 1280x704 direct at 2x, or very long clips) risks exhausting 16GB of VRAM or tripping the ring-timeout watchdog — the token-count warning dialog exists to flag that before you commit GPU time to a run that's likely to fail.
...
System RAM is no longer the limiting factor on a 124GB machine (peak usage is roughly 45GB — the resident 18GB transformer plus a transient text-encoder subprocess), so a large swap file is optional insurance rather than a hard requirement.
