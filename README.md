...
* **Full GPU-resident mode on large cards:** by default the transformer streams block-by-block from pinned host RAM (group offload), which is what a 16GB card needs. On a card with **≥29GB VRAM** (an estimate — see `FULL_RESIDENT_VRAM_THRESHOLD_GB` in `ltx_engine.py`; **not yet measured on real hardware**), it's kept fully GPU-resident instead, skipping the PCIe streaming cost entirely. Detected automatically from your card's reported VRAM — no setting to flip.
