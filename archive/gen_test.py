...
# =========================================================================
# 1. Environment Configuration (Must be set before PyTorch imports)
# =========================================================================
...
class CancellationError(Exception):
    """Custom exception raised when the user clicks Cancel."""
    pass

# =========================================================================
# 2. Hardware Monitoring (Zero-Dependency Linux Sysfs / Procfs Reader)
# =========================================================================
class LinuxHardwareMonitor:
    def __init__(self):
        ...
        # Discover primary AMD discrete GPU
        max_vram = 0
        for i in range(10):
    ...
    def get_ram_stats(self):
        try:
            ...
            for line in lines:
                parts = line.split(":")
                if len(parts) == 2:
                    mem_data[parts[0].strip()] = int(parts[1].strip().split()[0])
    ...
    def get_gpu_stats(self):
        ...
        try:
            ...
            return gpu_usage, vram_used / (1024**3), vram_total / (1024**3)
        except Exception:
            return None, None, None
...
# =========================================================================
# 3. Text Redirection (Routes terminal prints to Tkinter GUI)
# =========================================================================
...
# =========================================================================
# 4. Stage 1 Subprocess: Text Encoding
# =========================================================================
def encode_in_subprocess(model_path, p, np):
    ...
    print("  -> Encoding complete. Terminating process to forcefully release RAM.")
...
def generation_worker(config, root, progress_var, btn_generate, btn_cancel):
    ...
    try:
       ...
       try:
            ...
            pass  # Threads were already set during the first run
        
        # --- Stage 1: Text Subprocess ---
        print("\n--- [1/3] Loading Text Encoder & Encoding Prompts ---")
        ...
        while p_proc.is_alive():
            ...
            if cancel_flag:
                print("\n[!] Cancellation requested! Killing Text Encoder...")
                p_proc.terminate()
                p_proc.join()
        ...
        # --- Stage 2: Load Models ---
        print("--- [2/3] Loading Transformer/VAEs & Enabling VRAM Protections ---")
        ...
        print(f"--- [3/3] Generating {config['width']}x{config['height']} Video ({config['frames']} frames) ---")
        ...
        # --- Stage 4: Export ---
        output_file = f"output_{config['width']}x{config['height']}_{config['frames']}f_seed{config['active_seed']}.mp4"
...
def main():
    ...
    lbl_ram = tk.Label(primary_metrics, text="RAM: -- / -- GB", bg="#1a1a1a", fg="#ffffff", font=("Consolas", 10, "bold"), width=20)
    ...
    # Per-Core Matrix Box (High Contrast)
    ...
    def reset_np():
        ...
        text_np.insert(tk.END, "Loading default prompt from library...")
        root.update() 
        try:
    ...
    # Resolution Frame
    res_frame = ttk.LabelFrame(settings_frame, text=" Resolution ", padding="8")
    ...
    def start_generation():
        ...
        try:
            ...
            if not np_val:
                print("[*] Loading default negative prompt...")
                try:
                    from diffusers.pipelines.ltx2.utils import DEFAULT_NEGATIVE_PROMPT
            ...
            if s_val == 'r':
                ...
                print(f"\n[*] Generated Random Seed: {active_seed}")
            else:
                active_seed = int(s_val)
